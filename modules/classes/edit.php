...
$row = $pdo->prepare('SELECT * FROM kelas WHERE id = ?');
$row->execute([$id]);
$kelas = $row->fetch();

if (!$kelas) { header('Location: index.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($namaKelas !== '') {
        ...
        $chk->execute([$namaKelas, $id]);
        if ($chk->fetch()) $errors[] = 'Nama kelas sudah digunakan.';
    }

    if (empty($errors)) {
        $upd = $pdo->prepare('
            UPDATE kelas SET nama_kelas=?, tingkat=?, tahun_ajaran=?, wali_kelas_id=?
            WHERE id=?
        ');
        $upd->execute([$namaKelas, $tingkat, $tahunAjaran, $waliKelasId, $id]);
        ...
        <main class="flex-1 p-6 overflow-y-auto">
            ...
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    ...
                    <p class="text-gray-400 text-sm">Ubah data <?= htmlspecialchars($kelas['nama_kelas']) ?></p>
                </div>
            </div>
                ...
                <form method="POST" class="space-y-5" novalidate>
                    ...
                    <div class="grid grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="tahun_ajaran" value="<?= htmlspecialchars($kelas['tahun_ajaran'] ?? '') ?>"
